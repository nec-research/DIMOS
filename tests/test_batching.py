...
for idx in range(num_sim):
    system = dimos.AmberForceField(dimos.project_path("examples/amber/ala2_solv.prmtop"), nonbonded_type="Cutoff")

    systems.append(system)
    positions = dimos.utils.read_positions(dimos.project_path("examples/amber/ala2_solv.rst7"))

    integrator = dimos.LangevinDynamics(0.5, 200+20*idx, 0.1, system)
    simulation = dimos.MDSimulation(system, integrator, positions, temperature=200, seed=42)
    simulations.append(simulation)

#batched_sys = multi_sys(simulations)
batched_sys_all_different = dimos.MultiSys(systems)

all_pos = torch.cat([sim.pos for sim in simulations])
all_neighborlist = torch.cat([sim.neighborlist for sim in simulations], dim=1)
energy = 0
for sim in simulations:
    energy += sim.sys.calc_energy(positions, sim.neighborlist)
print(energy, batched_sys_all_different.calc_energy(all_pos, all_neighborlist))

batched_sim = dimos.MultiSim(simulations)

batched_sim.step(100)

new_neighborlist = batched_sim.get_combined_torch_neigborlist()
print(batched_sys_all_different.calc_energy(batched_sim.pos, new_neighborlist))
