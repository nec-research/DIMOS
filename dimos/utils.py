...
This module provides various helper functions used throughout DIMOS, including:
...
- Temperature and kinetic energy calculations
- File path handling
...
def get_distances_edge_list(pos: torch.Tensor, edge_list: torch.Tensor, periodic: bool, box: torch.Tensor) -> torch.Tensor:
    """
    Calculate distances between pairs of atoms specified by edge list.
    ...
    edge_list : torch.Tensor
        List of atom pairs to calculate distances between.
    periodic : bool
    ...
    """
    pos_dist = pos[edge_list[0]] - pos[edge_list[1]]
    if periodic and box is not None:
        pos_dist = periodic_correction(pos_dist, box)
    return torch.linalg.vector_norm(pos_dist, dim=1)


def get_distances_and_shifts(pos: torch.Tensor, edge_list: torch.Tensor, periodic: bool, box: torch.Tensor) -> torch.Tensor:
    """
    Calculate distances and periodic shifts between pairs of atoms.
    ...
    -------
    tuple
        Distances between atom pairs and their periodic shifts.
    """
    pos_dist = pos[edge_list[0]] - pos[edge_list[1]]
    if periodic and box is not None:
        shifts = torch.floor(pos_dist / box + 0.5) * box
        pos_dist = pos_dist - shifts
    else:
        shifts = torch.zeros((edge_list[0].size(0), 3))

    return torch.linalg.vector_norm(pos_dist, dim=1), shifts


def get_distance_vectors(pos: torch.Tensor, edge_list: torch.Tensor, periodic: bool, box: torch.Tensor) -> torch.Tensor:
    """
    Calculate distance vectors between pairs of atoms.
...
def get_distances_and_vectors(pos: torch.Tensor, edge_list: torch.Tensor, periodic: bool, box: torch.Tensor) -> torch.Tensor:
    """
    Calculate both distances and distance vectors between pairs of atoms.
    ...
    -------
    tuple
        Distances and distance vectors between atom pairs.
    """
    pos_dist = pos[edge_list[0]] - pos[edge_list[1]]
    if periodic:
        pos_dist = periodic_correction(pos_dist, box)
    return torch.linalg.vector_norm(pos_dist, dim=1), pos_dist


def periodic_correction(vectors, box):
    """
    Apply periodic boundary corrections to vectors.
...
def get_dihedral_angle(pos, edge_list_1, edge_list_2, edge_list_3, periodic, box):
    ...
    edge_list_1 : torch.Tensor
        First set of atom pairs.
    edge_list_2 : torch.Tensor
        Second set of atom pairs.
    edge_list_3 : torch.Tensor
        Third set of atom pairs.
    periodic : bool
        Whether to use periodic boundary conditions.
    box : torch.Tensor
        Box vectors for periodic systems.
    ...
    crossv1 = torch.linalg.cross(v1, v2, dim=1)
    crossv2 = torch.linalg.cross(v2, v3, dim=1)
    crossv3 = torch.linalg.cross(v2, crossv1, dim=1)

    normv1 = torch.linalg.vector_norm(crossv1, dim=1)
    normv2 = torch.linalg.vector_norm(crossv2, dim=1)
    normv3 = torch.linalg.vector_norm(crossv3, dim=1)
...
def maxwell_boltzmann_distr(sys, vel, temperature, generator=None, selection=None):
    """
    Generate velocities according to the Maxwell-Boltzmann distribution.

    Parameters
    ----------
    sys : object
        The system object containing information about the molecular system, including masses of the atoms.
    vel : torch.Tensor
        Tensor containing the initial velocities of the atoms.
    temperature : float
        The temperature at which the velocities should be generated.
    generator : torch.Generator, optional
        A random number generator for reproducibility. Default is None.
    selection : torch.Tensor, optional
        A tensor to select specific atoms for velocity generation. Default is None.

    Returns
    -------
    torch.Tensor
        Tensor containing the velocities of the atoms according to the Maxwell-Boltzmann distribution.

    Notes
    -----
    If `selection` is provided, velocities will be generated only for the selected atoms.
    """
    velocity_scale = torch.sqrt(constants.BOLTZMANN * temperature / sys.masses).unsqueeze(-1)
    random_vel = velocity_scale * torch.empty_like(vel).normal_(generator=generator)
    if selection is not None:
        return torch.where(~selection.unsqueeze(-1), vel, random_vel)
    return random_vel


def kinetic_energy(vel, sys):
    """
    ...
    sys : object
        System object containing atomic masses.
    ...
    """
    return 0.5 * torch.sum(sys.masses * torch.sum(vel * vel, dim=1))


def measure_temperature(vel, sys, num_constraints=0):
    """
    Measure the temperature of the system based on the velocities.
    ...
    num_constraints : int, optional
        Number of constraints in the system. Default is 0.
    ...
    Notes
    -----
    This function assumes a 3-dimensional system and subtracts 3 degrees of freedom (dof) 
    for the center of mass motion plus any additional constraints.
    """
    dof = sys.num_atoms * 3 - 3 - num_constraints
    return 2 * kinetic_energy(vel, sys) / (dof * constants.BOLTZMANN)


def read_positions(position_file):
    """
    Read atomic positions from a file using parmed.
    ...
    -------
    torch.Tensor
        Atomic positions in the appropriate units.

    Raises
    ------
    Exception
        If using unitless system.
    NotImplementedError
        If unit system is not recognized.
    """
    parmed_file_object = parmed.load_file(position_file)
    match constants.unit_system:
        case "kcal/mol" | "amber":
            return torch.tensor(parmed_file_object.positions.value_in_unit(parmed.unit.angstrom))
        case 'kJ/mol' | "openmm" | "gromacs":
            return torch.tensor(parmed_file_object.positions.value_in_unit(parmed.unit.nanometer))
        case "lj" | "none" | "unitless":
            raise Exception("For a unitless system, reading files via parmed is not too well defined. Please handle this yourself.")
        case _:
            raise NotImplementedError(f"Specified unit system not recognized: {constants.unit_system}")


def package_path(*paths, package_directory=os.path.dirname(os.path.abspath(__file__))):
    """
    Construct path relative to the package directory.

    Parameters
    ----------
    *paths : str
        Path components to join.
    package_directory : str, optional
        Base package directory. Default is current file's directory.
...
def project_path(*paths, project_directory=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))):
    """
    Construct path relative to the project root directory.
    ...
    *paths : str
    ...
    project_directory : str, optional
        Project root directory. Default is two levels up from current file.
