...
This module provides classes for handling molecular constraints in simulations. It includes
implementations of SETTLE for rigid water molecules and CCMA (Constant Constraint Matrix 
Approximation) for general distance constraints.

The module contains the following main classes:
    - PassthroughConstraintHandling: Dummy class for systems without constraints
    - ConstraintsHandling: Main class for managing all constraints in a system
    - CCMA: Implementation of the CCMA algorithm for distance constraints
    - Settle: Implementation of SETTLE algorithm for rigid water molecules

References
----------
SETTLE algorithm:
Miyamoto & Kollman, J. Comp. Chem. 13, 952-962 (1992)

CCMA algorithm:
Eastman & Pande, J. Chem. Theory Comput. 434-437 (2010)
"""

import torch

from dimos.utils import periodic_correction

class PassthroughConstraintHandling():
    """
    Dummy class for systems without constraints.
    
    This class provides a no-op implementation of constraint methods for use when
    a system has no constraints, avoiding the need for conditional logic elsewhere.
    """
    def __init__(self):
        self.num_total_constraints = 0

    def apply_constraints(self, pos, pos_prime):
        """No-op method for constraint application."""
        pass

    def apply_velocity_constraints(self, pos, vel):
        """No-op method for velocity constraint application."""
        pass

    def update_box(self, box):
        """No-op method for box updates."""
        pass


class ConstraintsHandling():
    """
    Main class for handling molecular constraints in a simulation.

    This class manages both SETTLE constraints for water molecules and CCMA constraints
    for other distance constraints. It automatically detects water molecules and uses
    the more efficient SETTLE algorithm for them while handling other constraints with
    CCMA.

    Parameters
    ----------
    constraints : list
        List of atom pairs to be constrained
    constraints_equilibrium : list
        Equilibrium distances for each constraint
    masses : list
        Atomic masses for all atoms
    num_atoms : int
        Total number of atoms in system
    angle_list : list
        List of angle definitions (needed for CCMA)
    angle_parameters : list
        Parameters for angles (needed for CCMA)
    tolerance : float
        Convergence tolerance for constraints
    max_num_iterations : int, optional
        Maximum iterations for constraint solving (default: 150)
    ...
    num_settle_constraints : int
        Number of SETTLE-handled constraints
    num_ccma_constraints : int
        Number of CCMA-handled constraints
    num_total_constraints : int
        Total number of constraints
    settle : Settle or None
        SETTLE handler if water molecules present
    ccma : CCMA or None
        CCMA handler if other constraints present
    """
    ...
    def __init__(
            ...
            periodic=False,
            box=None):
        """Initialize the ConstraintsHandling class."""
        ...
        atom_1 = []
        distance = []
        count_of_constraints = torch.zeros(num_atoms)
        for idx, constraint in enumerate(constraints):
            atom_idx_0 = constraint[0]
            atom_idx_1 = constraint[1]
            count_of_constraints[atom_idx_0] = count_of_constraints[atom_idx_0] + 1
            count_of_constraints[atom_idx_1] = count_of_constraints[atom_idx_1] + 1
            atom_0.append(atom_idx_0)
            atom_1.append(atom_idx_1)
            distance.append(constraints_equilibrium[idx])

        settle_constraints = []
        for i in range(num_atoms):
            settle_constraints.append([])

        counter = 0
        for i in range(len(atom_0)):
            if count_of_constraints[atom_0[i]] == 2 and count_of_constraints[atom_1[i]] == 2:
                counter += 1
                settle_constraints[atom_0[i]].append((atom_1[i], distance[i]))
                settle_constraints[atom_1[i]].append((atom_0[i], distance[i]))

        settle_clusters = []
        for idx, settle_constraint in enumerate(settle_constraints):
            if len(settle_constraint) == 2:
                partner1 = settle_constraint[0][0]
                partner2 = settle_constraint[1][0]
                if len(settle_constraints[partner1]) != 2 or len(settle_constraints[partner2]) != 2:
                    settle_constraint = []
                elif idx < partner1 and idx < partner2:
                    settle_clusters.append(idx)
            else:
                settle_constraint = []
        ...
        distance0 = []
        distance1 = []
        is_settle_atom = torch.zeros(num_atoms, dtype=torch.bool)

        for cluster in settle_clusters:
            particle0 = cluster
            particle1 = settle_constraints[particle0][0][0]
            particle2 = settle_constraints[particle0][1][0]
            dist01 = settle_constraints[particle0][0][1]
            dist02 = settle_constraints[particle0][1][1]
            if settle_constraints[particle1][0][0] == particle2:
                dist12 = settle_constraints[particle1][0][1]
            else:
                dist12 = settle_constraints[particle1][1][1]
            if dist01 == dist02:
                atom0.append(particle0)
                atom1.append(particle1)
                atom2.append(particle2)
                distance0.append(dist01)
                distance1.append(dist12)
            elif dist01 == dist12:
                atom0.append(particle1)
                atom1.append(particle0)
                atom2.append(particle2)
                distance0.append(dist01)
                distance1.append(dist02)
            elif dist02 == dist12:
                atom0.append(particle2)
                atom1.append(particle0)
                atom2.append(particle1)
                distance0.append(dist02)
                distance1.append(dist01)
            else:
                continue
            is_settle_atom[particle0] = True
            is_settle_atom[particle1] = True
            is_settle_atom[particle2] = True

        self.num_settle_constraints = len(atom0)
        self.settle = None
        if self.num_settle_constraints > 0:
            self.settle = Settle(atom0, atom1, atom2, distance0, distance1, masses, periodic=periodic, box=box)

        self.settle_atom0 = atom0
        self.settle_atom1 = atom1
        self.settle_atom2 = atom2
        self.settle_distance0 = distance0
        self.settle_distance1 = distance1

        ccmaConstraints = []
        for i in range(len(atom_0)):
            if not is_settle_atom[atom_0[i]]:
                ccmaConstraints.append(i)
        self.num_ccma_constraints = len(ccmaConstraints)

        self.ccma = None
        if self.num_ccma_constraints > 0:
            ccmaIndices = []
            ccmaDistance = []
            for i in range(self.num_ccma_constraints):
                ccmaIndices.append([])
                ccmaDistance.append([])

            for i in range(self.num_ccma_constraints):
                index = ccmaConstraints[i]
                ccmaIndices[i] = [atom_0[index], atom_1[index]]
                ccmaDistance[i] = distance[index]

            self.ccma = CCMA(
                self.num_ccma_constraints,
                ccmaIndices,
                ccmaDistance,
                angle_list,
                angle_parameters,
                ...
                periodic,
                box)
            self.ccma_indices = ccmaIndices
            self.ccma_distances = ccmaDistance

        self.num_total_constraints = self.num_ccma_constraints + self.num_settle_constraints
    ...
    def apply_constraints(self, pos, pos_prime):
        """
        Apply all constraints to a set of positions.

        Parameters
        ----------
        pos : torch.Tensor
            Reference positions
        pos_prime : torch.Tensor
            Positions to be constrained
        """
        if self.settle is not None:
            self.settle.apply_constraints(pos, pos_prime)
        if self.ccma is not None:
            self.ccma.apply_constraints(pos, pos_prime)

    def apply_velocity_constraints(self, pos, vel):
        """
        Apply all constraints to velocities.
        ...
        pos : torch.Tensor
            Current positions
        vel : torch.Tensor
            Velocities to be constrained
        """ 
        if self.settle is not None:
            self.settle.apply_velocity_constraints(pos, vel)     
        if self.ccma is not None:
            self.ccma.apply_velocity_constraints(pos, vel)        


class CCMA():
    """
    Implementation of the CCMA algorithm for distance constraints.

    This class implements the Constant Constraint Matrix Approximation algorithm for
    handling general distance constraints in molecular systems. It provides efficient
    constraint solving by approximating the Jacobian matrix as constant.
    ...
    indices : list
        List of atom index pairs for each constraint
    distances : list
        Equilibrium distances for each constraint
    angles : list
        List of angle definitions
    angles_parameters : torch.Tensor
    ...
    periodic : bool, optional
        Whether to use periodic boundaries (default: False)
    box : torch.Tensor, optional
        Box vectors if using periodic boundaries
    ...
    indices : torch.Tensor
        Constraint atom pairs
    distances : torch.Tensor
        Constraint distances
    dist2 : torch.Tensor
        Squared constraint distances
    inverse_masses : torch.Tensor
        Inverse atomic masses
    reduced_masses : torch.Tensor
        Reduced masses for constraints
    tolerance : float
        Convergence tolerance
    ...
    inverse_matrix : torch.Tensor
        Inverse of constraint matrix
    row_indices : torch.Tensor
        Row indices for sparse operations
    col_indices : torch.Tensor
        Column indices for sparse operations
    inverse_matrix_values : torch.Tensor
        Values for sparse operations
    """
    ...
    def __init__(
            ...
            periodic=False,
            box=None):
        """Initialize the CCMA class."""
        self.periodic = periodic
        self.box = box

        self.indices = torch.tensor(indices, dtype=torch.int64)
        self.distances = torch.tensor(distances)
        self.dist2 = self.distances * self.distances

        self.angles = angles
        self.angles_parameters = angles_parameters

        self.num_constraints = num_constraints
        self.max_num_iterations = max_num_iterations

        self.inverse_masses = 1.0 / masses
        self.reduced_masses = 0.5 / (self.inverse_masses[self.indices[:, 0]] + self.inverse_masses[self.indices[:, 1]])

        self.num_atoms = num_atoms

        self.tolerance = tolerance
        self.lowerTolerance = 1 - 2 * tolerance + tolerance * tolerance
        self.upperTolerance = 1 + 2 * tolerance + tolerance * tolerance

        matrix = torch.zeros((self.num_constraints, self.num_constraints))

        atomAngles = []
        atomConstraints = []
        for _ in range(self.num_atoms):
            atomAngles.append([])
            atomConstraints.append(set())

        for idx, angle in enumerate(self.angles):
            atomAngles[angle[1]].append(idx)

        for idx, indice in enumerate(self.indices):
            atomConstraints[indice[0]].add(idx)
            atomConstraints[indice[1]].add(idx)

        for j in range(self.num_constraints):
            atomj0 = self.indices[j, 0]
            atomj1 = self.indices[j, 1]
            invMass0 = self.inverse_masses[atomj0]
            invMass1 = self.inverse_masses[atomj1]
            constraints = atomConstraints[atomj0] | atomConstraints[atomj1]
            for k in constraints:
                if j == k:
                    matrix[j, k] = 1.0
                    continue
                atomk0 = self.indices[k, 0]
                atomk1 = self.indices[k, 1]
                if atomj0 == atomk0:
                    atoma = atomj1
                    atomb = atomj0
                    atomc = atomk1
                    ...
                    continue  # non connected constraints

                # check for a third constraint
                for other in atomConstraints[atoma]:
                    if self.indices[other, 0] == atomc or self.indices[other, 1] == atomc:
                        d1 = self.distances[j]
                        d2 = self.distances[k]
                        d3 = self.distances[other]
                        matrix[j, k] = scale * (d1 * d1 + d2 * d2 - d3 * d3) / (2.0 * d1 * d2)
                        break
                else:
                    angle_candidates = atomAngles[atomb]
                    for cand in angle_candidates:
                        angle_cand = self.angles[cand]
                        if (angle_cand[0] == atoma and angle_cand[2] == atomc) or (angle_cand[2] == atoma and angle_cand[0] == atomc):
                            matrix[j, k] = scale * torch.cos(self.angles_parameters[cand, 1])
                            break

        # Alternatively, inverse may be calculated via QR decomposition.
        self.inverse_matrix = matrix.inverse().T
        self.row_indices, self.col_indices = self.inverse_matrix.nonzero(as_tuple=True)
        self.inverse_matrix_values = self.inverse_matrix[self.row_indices, self.col_indices]

    def apply_constraints(self, pos, pos_prime):
        """
        Apply CCMA constraints to positions.
        ...
        r_ij = pos[self.indices[:, 0]] - pos[self.indices[:, 1]]
        if self.periodic:
            r_ij = periodic_correction(r_ij, self.box)

        counter = 0
        for _ in range(self.max_num_iterations):
            counter += 1
            rp_ij = pos_prime[self.indices[:, 0]] - pos_prime[self.indices[:, 1]]
            if self.periodic:
                rp_ij = periodic_correction(rp_ij, self.box)

            rp2 = torch.linalg.vecdot(rp_ij, rp_ij)

            num_converged = ((rp2 >= (self.lowerTolerance * self.dist2)) & (rp2 <= (self.upperTolerance * self.dist2))).sum()

            if num_converged == self.num_constraints:
                break

            diff = self.dist2 - rp2
            rrpr = torch.linalg.vecdot(rp_ij, r_ij)

            constraint_delta = self.reduced_masses * diff / rrpr

            contributions = self.inverse_matrix_values * constraint_delta[self.col_indices]

            constraint_delta.zero_()
            constraint_delta.index_add_(0, self.row_indices, contributions)

            dr = r_ij * constraint_delta.unsqueeze(1)
            update_0 = dr * self.inverse_masses[self.indices[:, 0]].unsqueeze(1)
            update_1 = dr * self.inverse_masses[self.indices[:, 1]].unsqueeze(1)

            pos_prime.index_add_(0, self.indices[:, 0], update_0)
            pos_prime.index_add_(0, self.indices[:, 1], -update_1)

    def apply_velocity_constraints(self, pos, vel):
        """
        Apply CCMA constraints to velocities.
        ...
        d_ij2 = torch.linalg.vecdot(r_ij, r_ij)

        counter = 0
        for _ in range(self.max_num_iterations):
            counter += 1
            rp_ij = vel[self.indices[:, 0]] - vel[self.indices[:, 1]]
            rrpr = torch.linalg.vecdot(rp_ij, r_ij)

            constraint_delta = -2.0 * self.reduced_masses * rrpr / d_ij2
            num_converged = torch.sum(torch.abs(constraint_delta) <= self.tolerance)
            ...
            vel.index_add_(0, self.indices[:, 0], update_0)
            vel.index_add_(0, self.indices[:, 1], -update_1)


class Settle():
    """
    Implementation of the SETTLE algorithm for rigid water molecules.

    This class implements the SETTLE algorithm for efficiently maintaining rigid
    constraints in water molecules. It provides both position and velocity constraint
    handling.

    Parameters
    ----------
    atom0 : list
        List of oxygen atom indices
    atom1 : list
        List of first hydrogen atom indices
    atom2 : list
        List of second hydrogen atom indices
    distance0 : list
        O-H bond distances
    distance1 : list
        H-H distances
    ...
    atom0 : torch.Tensor
        Oxygen atom indices
    atom1 : torch.Tensor
        First hydrogen indices
    atom2 : torch.Tensor
        Second hydrogen indices
    distance0 : torch.Tensor
        O-H distances
    ...
    mA : torch.Tensor
        Oxygen masses
    mB : torch.Tensor
        First hydrogen masses
    mC : torch.Tensor
        Second hydrogen masses
    inverse_total_mass : torch.Tensor
        Inverse total masses
    rc : torch.Tensor
        Half H-H distances
    rb : torch.Tensor
        Derived geometric parameter
    ra : torch.Tensor
        Derived geometric parameter
    """

    def __init__(self, atom0, atom1, atom2, distance0, distance1, masses, tolerance=1e-4, periodic=False, box=None):
        """Initialize the Settle class."""
        self.atom0 = torch.tensor(atom0)
        self.atom1 = torch.tensor(atom1)
        self.atom2 = torch.tensor(atom2)
        self.distance0 = torch.tensor(distance0)  # *0.1 #!!! TODO DEBUG
        self.distance1 = torch.tensor(distance1)  # *0.1
        self.masses = masses
        self.tolerance = tolerance
        self.mA = self.masses[self.atom0]
        self.mB = self.masses[self.atom1]
        self.mC = self.masses[self.atom2]
        self.periodic = periodic
        self.box = box

        self.inverse_total_mass = 1 / (self.mA + self.mB + self.mC)
        self.rc = 0.5 * self.distance1
        self.rb = torch.sqrt(self.distance0 * self.distance0 - self.rc * self.rc)
        self.ra = self.rb * (self.mB + self.mC) * self.inverse_total_mass
        self.rb -= self.ra
    ...
    def apply_constraints(self, pos, pos_prime):
        ...
        apos0 = pos[self.atom0]
        apos1 = pos[self.atom1]
        apos2 = pos[self.atom2]

        xp0 = pos_prime[self.atom0] - apos0
        xp1 = pos_prime[self.atom1] - apos1
        xp2 = pos_prime[self.atom2] - apos2

        if self.periodic:
            xp0 = periodic_correction(xp0, self.box)
            xp1 = periodic_correction(xp1, self.box)
            xp2 = periodic_correction(xp2, self.box)

        m0 = self.mA.unsqueeze(1)
        m1 = self.mB.unsqueeze(1)
        m2 = self.mC.unsqueeze(1)

        b0 = apos1 - apos0
        c0 = apos2 - apos0

        if self.periodic:
            b0 = periodic_correction(b0, self.box)
            c0 = periodic_correction(c0, self.box)

        com = (xp0 * m0 + (b0 + xp1) * m1 + (c0 + xp2) * m2) * self.inverse_total_mass.unsqueeze(1)

        a1 = xp0 - com
        b1 = b0 + xp1 - com
        c1 = c0 + xp2 - com
        if self.periodic:
            a1 = periodic_correction(a1, self.box)
            b1 = periodic_correction(b1, self.box)
            c1 = periodic_correction(c1, self.box)

        aksZd = torch.linalg.cross(b0, c0)
        aksXd = torch.linalg.cross(a1, aksZd)
        aksYd = torch.linalg.cross(aksZd, aksXd)

        trns1 = torch.nn.functional.normalize(aksXd, dim=-1)
        trns2 = torch.nn.functional.normalize(aksYd, dim=-1)
        trns3 = torch.nn.functional.normalize(aksZd, dim=-1)

        xb0d = torch.linalg.vecdot(trns1, b0)
        yb0d = torch.linalg.vecdot(trns2, b0)
        xc0d = torch.linalg.vecdot(trns1, c0)
        yc0d = torch.linalg.vecdot(trns2, c0)
        za1d = torch.linalg.vecdot(trns3, a1)
        xb1d = torch.linalg.vecdot(trns1, b1)
        yb1d = torch.linalg.vecdot(trns2, b1)
        zb1d = torch.linalg.vecdot(trns3, b1)
        xc1d = torch.linalg.vecdot(trns1, c1)
        yc1d = torch.linalg.vecdot(trns2, c1)
        zc1d = torch.linalg.vecdot(trns3, c1)

        # Step 2
        sinphi = za1d / self.ra
        cosphi = torch.sqrt(1 - sinphi * sinphi)
        sinpsi = (zb1d - zc1d) / (2 * self.rc * cosphi)
        cospsi = torch.sqrt(1 - sinpsi * sinpsi)

        ya2d = self.ra * cosphi
        xb2d = -  self.rc * cospsi
        yb2d = -  self.rb * cosphi - self.rc * sinpsi * sinphi
        yc2d = -  self.rb * cosphi + self.rc * sinpsi * sinphi
        xb2d2 = xb2d * xb2d
        hh2 = 4.0 * xb2d2 + (yb2d - yc2d) * (yb2d - yc2d) + (zb1d - zc1d) * (zb1d - zc1d)
        deltx = 2.0 * xb2d + torch.sqrt(4.0 * xb2d2 - hh2 + self.distance1 * self.distance1)
        xb2d = xb2d - 0.5 * deltx
        ...
        sintheta = (alpha * gamma - beta * torch.sqrt(al2be2 - gamma * gamma)) / al2be2

        # Step 4
        costheta = torch.sqrt(1 - sintheta * sintheta)
        xa3d = -ya2d * sintheta
        ya3d = ya2d * costheta
        za3d = za1d
        ...
        # Step 5
        xa3 = trns1[:, 0] * xa3d + trns2[:, 0] * ya3d + trns3[:, 0] * za3d
        ya3 = trns1[:, 1] * xa3d + trns2[:, 1] * ya3d + trns3[:, 1] * za3d
        za3 = trns1[:, 2] * xa3d + trns2[:, 2] * ya3d + trns3[:, 2] * za3d

        xb3 = trns1[:, 0] * xb3d + trns2[:, 0] * yb3d + trns3[:, 0] * zb3d
        yb3 = trns1[:, 1] * xb3d + trns2[:, 1] * yb3d + trns3[:, 1] * zb3d
        zb3 = trns1[:, 2] * xb3d + trns2[:, 2] * yb3d + trns3[:, 2] * zb3d

        xc3 = trns1[:, 0] * xc3d + trns2[:, 0] * yc3d + trns3[:, 0] * zc3d
        yc3 = trns1[:, 1] * xc3d + trns2[:, 1] * yc3d + trns3[:, 1] * zc3d
        zc3 = trns1[:, 2] * xc3d + trns2[:, 2] * yc3d + trns3[:, 2] * zc3d

        a3 = torch.stack((xa3, ya3, za3)).T
        b3 = torch.stack((xb3, yb3, zb3)).T
        c3 = torch.stack((xc3, yc3, zc3)).T

        xp0 = com + a3
        xp1 = com + b3 - b0
        xp2 = com + c3 - c0

        pos_prime[self.atom0] = xp0 + apos0
        pos_prime[self.atom1] = xp1 + apos1
        pos_prime[self.atom2] = xp2 + apos2

    def apply_velocity_constraints(self, pos, vel):
        """
        Apply SETTLE constraints to the velocities.
        ...
        if self.periodic:
            eAB = periodic_correction(eAB, self.box)
            eBC = periodic_correction(eBC, self.box)
            eCA = periodic_correction(eCA, self.box)

        eAB = torch.nn.functional.normalize(eAB, p=2, dim=-1)
        eBC = torch.nn.functional.normalize(eBC, p=2, dim=-1)
        eCA = torch.nn.functional.normalize(eCA, p=2, dim=-1)

        vAB = torch.linalg.vecdot((v1 - v0), eAB)
        vBC = torch.linalg.vecdot((v2 - v1), eBC)
        vCA = torch.linalg.vecdot((v0 - v2), eCA)

        cA = -torch.linalg.vecdot(eAB, eCA)
        cB = -torch.linalg.vecdot(eAB, eBC)
        cC = -torch.linalg.vecdot(eBC, eCA)

        s2A = 1.0 - cA * cA  # Unitless
        s2B = 1.0 - cB * cB
        s2C = 1.0 - cC * cC

        mABCinv = 1 / (self.mA * self.mB * self.mC)
        denom = ((((s2A * self.mB + s2B * self.mA) * self.mC + (s2A * self.mB * self.mB + 2 * (cA * cB * cC + 1) * self.mA *
                 self.mB + s2B * self.mA * self.mA)) * self.mC + s2C * self.mA * self.mB * (self.mA + self.mB)) * mABCinv)
        tab = ((cB * cC * self.mA - cA * self.mB - cA * self.mC) * vCA + (cA * cC * self.mB - cB * self.mC - cB * self.mA)
               * vBC + (s2C * self.mA * self.mA * self.mB * self.mB * mABCinv + (self.mA + self.mB + self.mC)) * vAB) / denom
        tbc = ((cA * cB * self.mC - cC * self.mB - cC * self.mA) * vCA + (s2A * self.mB * self.mB * self.mC * self.mC *
               mABCinv + (self.mA + self.mB + self.mC)) * vBC + (cA * cC * self.mB - cB * self.mA - cB * self.mC) * vAB) / denom
        tca = ((s2B * self.mA * self.mA * self.mC * self.mC * mABCinv + (self.mA + self.mB + self.mC)) * vCA + (cA * cB *
               self.mC - cC * self.mB - cC * self.mA) * vBC + (cB * cC * self.mA - cA * self.mB - cA * self.mC) * vAB) / denom

        v0 = v0 + (eAB * tab.unsqueeze(1) - eCA * tca.unsqueeze(1)) / self.mA.unsqueeze(1)
        v1 = v1 + (eBC * tbc.unsqueeze(1) - eAB * tab.unsqueeze(1)) / self.mB.unsqueeze(1)
        v2 = v2 + (eCA * tca.unsqueeze(1) - eBC * tbc.unsqueeze(1)) / self.mC.unsqueeze(1)
