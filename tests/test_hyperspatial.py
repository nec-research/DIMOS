...
import dimos
import dimos.advanced_methods
import dimos.advanced_methods.hyperspatial_utils
import torch

torch.manual_seed(42)
torch.set_default_dtype(torch.float64)
torch.set_default_device("cpu")
prmtop_filename = "./examples/amber/ala2_solv.prmtop"
rst7_filename = "./examples/amber/ala2_solv.rst7"

positions = dimos.read_positions(rst7_filename)
positions.requires_grad_()
system = dimos.ff.AmberForceField(prmtop_filename, nonbonded_type="NoCutoff", dispersion_correction=False)

energy, force, _ = system.calc_energy(positions, None, print_energies=True, return_forces=True)

extra_dim = 1
mu = 2.0
noise = 0.5

ext_positions = torch.nn.functional.pad(positions, (0, extra_dim), value=0.0)
ext_positions.requires_grad_()

ext_positions[:,3:] = torch.randn_like(ext_positions[:,3:]) * noise

separable_hyperspatial_system = dimos.advanced_methods.hyperspatial_utils.SeparableHyperspatialSystem(system, extra_dim, mu)
separable_energy, separable_force, _ = separable_hyperspatial_system.calc_energy(ext_positions, None, return_forces=True)

non_separable_hyperspatial_system = dimos.advanced_methods.hyperspatial_utils.HyperspatialSystem(system, extra_dim, mu)
non_separable_energy, non_separable_force, _ = non_separable_hyperspatial_system.calc_energy(ext_positions, None, return_forces=True, print_energies=True)

print("#### ENERGY: ####")
print("Separable", energy-separable_energy)
print("Non-Separable", energy-non_separable_energy)

print("#### FORCES: ####")
print("Separable", torch.sum(torch.abs(force-separable_force[:,:3])).item(), torch.sum(separable_force[:,3:]).item())
print("Non-Separable", torch.sum(torch.abs(force-non_separable_force[:,:3])).item(), torch.sum(non_separable_force[:,3:]).item())


integrator = dimos.integrators.LangevinDynamics(0.5, 300, 0.01, non_separable_hyperspatial_system)

simulation = dimos.simulation.MDSimulation(non_separable_hyperspatial_system, integrator, ext_positions, temperature=300.0)