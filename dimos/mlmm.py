...
"""
Mixed ML/MM molecular dynamics module.

This module provides functionality for hybrid molecular dynamics simulations that combine
classical molecular mechanics (MM) with machine learning (ML) potentials. It enables
treating specific atoms or regions with ML potentials while using classical force fields
for the rest of the system.

The module implements the MLMMsystem class which handles:
- Separation of ML and MM regions
- ML potential calculations for ML atoms
- Classical MM calculations for MM atoms and optionally ML-MM interactions
- Support for various electrostatic methods including PME
- Energy and force calculations for the combined system

The hybrid approach allows leveraging the accuracy of ML potentials for regions of
interest while maintaining the efficiency of classical force fields for the bulk system.
"""

import torch

from dimos.energy import LennardJones, PME
from dimos.utils import get_distances_edge_list
from dimos.ff import MinimalSystem

from warnings import warn


class MLMMsystem(MinimalSystem):
    """
    A class to handle mixed ML/MM molecular systems.

    This class combines classical molecular mechanics (MM) with machine learning (ML)
    potentials for hybrid simulations. It allows treating specific atoms with ML
    potentials while using classical force fields for the rest.

    Parameters
    ----------
    ml_atoms : list or torch.Tensor
        Indices of atoms to be treated with ML potentials
    ml_system : object
        The ML system object that handles ML potential calculations
    mm_system : MinimalSystem
        The classical MM system object
    treat_mlmm_classically : bool, optional
        Whether to treat ML-MM interactions using classical potentials (default True)
    electrostatic_type : str, optional
        Type of electrostatic calculation. Currently supports "PME" or None
    create_graph : bool, optional
        Whether to create computational graphs for autograd (default False)

    Attributes
    ----------
    ml_atoms : list or torch.Tensor
        Indices of atoms treated with ML potentials
    mm_system : MinimalSystem
        The classical MM system
    ml_system : object
        The ML system
    all_exclusions : torch.Tensor
        Excluded atom pairs for nonbonded interactions
    constraint_handler : object
        Handles distance constraints between atoms
    masses : torch.Tensor
        Atomic masses
    num_atoms : int
        Total number of atoms
    periodic : bool
        Whether system uses periodic boundary conditions
    box : torch.Tensor
    ...
    treat_mlmm_classically : bool
        Whether ML-MM interactions use classical potentials
    """

    def __init__(self, ml_atoms, ml_system, mm_system, treat_mlmm_classically=True, electrostatic_type=None, create_graph=False):
        super().__init__(mm_system.unit_system, create_graph, mm_system.dtype)
        self.ml_atoms = ml_atoms
        self.mm_system = mm_system
        self.ml_system = ml_system

        if self.mm_system.explicit_adjusts:
            warn("Ignoring explicit adjusts for the ML/MM modelling", stacklevel=2)

        self.all_exclusions = self.mm_system.all_exclusions
        self.constraint_handler = self.mm_system.constraint_handler
        self.masses = self.mm_system.masses
        self.num_atoms = self.mm_system.num_atoms
        self.periodic = self.mm_system.periodic
        self.box = self.mm_system.box
        self.use_neighborlist = self.mm_system.use_neighborlist
        self.cutoff = self.mm_system.cutoff
        self.treat_mlmm_classically = treat_mlmm_classically


        self.mm_system_nonbonded_lj = LennardJones(
            self.mm_system.sigmas,
            self.mm_system.epsilons,
            self.mm_system.cutoff,
            self.mm_system.switch_distance,
            self.mm_system.scnb,
            self.mm_system.dihedral_exclusions,
            self.mm_system.periodic,
            self.mm_system.box)

        match electrostatic_type:
            case "PME":
                tolerance = 5e-4
                self.mm_system_nonbonded_electrostatics = PME(
                    self.mm_system.charges,
                    tolerance,
                    self.mm_system.cutoff,
                    scee=self.mm_system.scee,
                    all_exclusions_without_improper=self.mm_system.all_exclusions,
                    dihedral_exclusions=self.mm_system.dihedral_exclusions,
                    periodic=self.mm_system.periodic,
                    box=self.mm_system.box)
            case _:
                self.mm_system_nonbonded_electrostatics = None
    ...
    def calc_energy(self, pos: torch.Tensor, neighborlist, return_forces: bool = False, print_energies=False):
        """
        Calculate the total energy of the hybrid ML/MM system.

        This method computes the total energy by:
        1. Separating interactions into ML-ML, ML-MM, and MM-MM pairs
        2. Computing ML energies for ML atoms
        3. Computing MM bonded and nonbonded energies
        4. Optionally computing forces and accelerations
        ...
        neighborlist : torch.Tensor
            List of neighboring atom pairs
        return_forces : bool, optional
            Whether to compute and return forces (default False)
        print_energies : bool, optional
            Whether to print energy components (default False)

        Returns
        -------
        torch.Tensor or tuple
            If return_forces is False, returns total energy
            If return_forces is True, returns (energy, forces, acceleration)
        """
        mask_ml_0 = torch.isin(neighborlist[0], self.ml_atoms)
        mask_ml_1 = torch.isin(neighborlist[1], self.ml_atoms)

        purely_ml_mask = mask_ml_0 & mask_ml_1
        partly_ml_mask = (mask_ml_0 | mask_ml_1) & ~purely_ml_mask
        purely_mm_mask = ~mask_ml_0 & ~mask_ml_1
        mm_and_partly_ml_mask = partly_ml_mask | purely_mm_mask 
        ml_and_partly_ml_mask = partly_ml_mask | purely_ml_mask
        
        ml_neighborlist = neighborlist[:, purely_ml_mask]
        mm_and_partly_ml_neighborlist = neighborlist[:, mm_and_partly_ml_mask]
        ml_and_partly_ml_neighborlist = neighborlist[:, ml_and_partly_ml_mask]
        mm_neighborlist = neighborlist[:, purely_mm_mask]

        #torch.cuda.synchronize()

        #with torch.cuda.stream(self.s1):
        if self.treat_mlmm_classically:
            ml_energy = self.ml_system.calc_energy(pos, ml_neighborlist, return_forces=False, print_energies=False)
        else:
            ml_energy = self.ml_system.calc_energy(pos, ml_and_partly_ml_neighborlist, return_forces=False, print_energies=False)
        # MM bonded
        mm_bonded_energy = self.mm_system.calc_energy(pos, mm_neighborlist, return_forces=False, print_energies=False)

        # MM nonbonded energy
        if self.mm_system_nonbonded_electrostatics is not None:
            distance_matrix = get_distances_edge_list(pos, neighborlist, self.mm_system.periodic, self.mm_system.box)
            within_cutoff = distance_matrix <= self.mm_system.cutoff
            #torch.cuda.synchronize()            
            #with torch.cuda.stream(self.s2):
            neighborlist_within_cutoff = neighborlist[:, within_cutoff]
            distance_matrix_cut = distance_matrix[within_cutoff]
            inverse_distance_cut = 1 / distance_matrix_cut

            electrostatic_energy = self.mm_system_nonbonded_electrostatics.calc_energy(
                pos, inverse_distance_cut, distance_matrix_cut, neighborlist_within_cutoff)
            #with torch.cuda.stream(self.s3):
            if self.treat_mlmm_classically:
                within_cutoff_lj = within_cutoff & mm_and_partly_ml_mask
            else:
                within_cutoff_lj = within_cutoff & purely_mm_mask
            neighborlist_within_cutoff_lj = neighborlist[:, within_cutoff_lj]
            distance_matrix_cut_lj = distance_matrix[within_cutoff_lj]
            inverse_distance_cut_lj = 1.0 / distance_matrix_cut_lj

            lennard_jones_energy = self.mm_system_nonbonded_lj.calc_energy(
                pos, inverse_distance_cut_lj, distance_matrix_cut_lj, neighborlist_within_cutoff_lj)
        else:
            #with torch.cuda.stream(self.s2):
            if self.treat_mlmm_classically:
                distance_matrix = get_distances_edge_list(pos, mm_and_partly_ml_neighborlist, self.mm_system.periodic, self.mm_system.box)
                within_cutoff = distance_matrix <= self.mm_system.cutoff
                mm_neighborlist_within_cutoff = mm_and_partly_ml_neighborlist[:, within_cutoff]
            else:
                distance_matrix = get_distances_edge_list(pos, mm_neighborlist, self.mm_system.periodic, self.mm_system.box)
                within_cutoff = distance_matrix <= self.mm_system.cutoff
                mm_neighborlist_within_cutoff = mm_neighborlist[:, within_cutoff]
            distance_matrix_cut = distance_matrix[within_cutoff]
            inverse_distance_cut = 1 / distance_matrix_cut

            electrostatic_energy = 0.0
            lennard_jones_energy = self.mm_system_nonbonded_lj.calc_energy(
                pos, inverse_distance_cut, distance_matrix_cut, mm_neighborlist_within_cutoff)

        #torch.cuda.synchronize()
        energy = mm_bonded_energy + lennard_jones_energy + electrostatic_energy + ml_energy
