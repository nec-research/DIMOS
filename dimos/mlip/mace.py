...
"""
MACE machine learning potential module.

This module provides integration with MACE machine learning potentials for molecular simulations. 

The module contains the following main classes:

- MaceFoundationalSystem: System class that implements MACE foundational models
- MaceEnergyContribution: Energy calculation class using MACE potentials

These classes enable molecular dynamics simulations using pre-trained MACE models,
with support for periodic boundary conditions, neighbor lists, and different unit systems.
The implementation includes options for CuEq acceleration and handles unit conversions
between MACE's native units (Angstroms, eV) and DIMOS internal units.
"""

import torch

from dimos.utils import get_distances_and_shifts
from dimos.ff import MinimalSystem
from dimos import constants

import mace.tools as macetools
from mace.cli.convert_e3nn_cueq import run as run_cueq


class MaceFoundationalSystem(MinimalSystem):
    """
    System class for using MACE foundational models.

    This class implements the MACE foundational models for molecular simulations,
    providing an interface between DIMOS and the MACE machine learning potentials.
    ...
    mace_name : str, optional
        Name of the MACE model to use, default is "mace-mp-small"
    atom_range : tuple, optional
        Range of atoms to include in calculations, default is None (all atoms)
    enable_cueq : bool, optional
        Whether to use CuEq acceleration, default is True
    periodic : bool, optional
        Whether to use periodic boundary conditions, default is False
    box : torch.Tensor, optional
        Box vectors for periodic systems, default is None
    use_neighborlist : bool, optional
        Whether to use neighbor lists, default is True
    unit_system : str, optional
        Unit system to use ("amber" etc), default is "amber"
    create_graph : bool, optional
        Whether to create computational graph for forces, default is False
    dtype : torch.dtype, optional
        Data type for tensors, default is torch default dtype
    ...
    use_neighborlist : bool
        Whether neighbor lists are used
    all_exclusions : torch.Tensor
        List of excluded interactions
    box : torch.Tensor
        Periodic box vectors
    cv_force : None
        Placeholder for collective variable forces
    ...
    def __init__(
            ...
            dtype=None):
        if dtype is None:
            super().__init__(unit_system, create_graph, torch.get_default_dtype())
        else:
            super().__init__(unit_system, create_graph, dtype)
        self.periodic = periodic
        self.masses = masses
        self.num_atoms = masses.size(0)
        ...
        mace_energy_contribution = MaceEnergyContribution(
            atomic_numbers,
        ...
        self.nonbonded_force_components.append(mace_energy_contribution)
        self.cutoff = mace_energy_contribution.model.r_max.cpu()


class MaceEnergyContribution():
    """
    Energy contribution class for MACE machine learning potentials.

    This class handles the energy calculations using MACE models, converting between
    different unit systems and managing periodic boundary conditions.
    ...
    energy_type : str, optional
        Type of energy to compute ('interaction_energy' or 'energy'), default is 'interaction_energy'
    box : torch.Tensor, optional
        Box vectors for periodic systems, default is None
    periodic : bool, optional
        Whether to use periodic boundary conditions, default is False
    ...
    enable_cueq : bool, optional
        Whether to use CuEq acceleration, default is True
    path : str, optional
        Path to custom MACE model, default is None
    dtype : torch.dtype, optional
        Data type for tensors, default is torch default dtype

    Notes
    -----
    MACE expects positions in Angstrom and returns energies in eV.
    Based on: https://github.com/openmm/openmm-ml/blob/main/openmmml/models/macepotential.py
    """

    def __init__(self,
                 atomic_numbers,
                 atom_range = None,
                 energy_type: str = 'interaction_energy',
                 box: torch.Tensor = None,
                 periodic: bool = False,
                 mace_name: str = 'mace-mp-small',
                 enable_cueq=True,
                 path: str = None,
                 dtype=None):
        ...
        assert energy_type in ["interaction_energy", "energy"], f"Return type must be 'interaction_energy' or 'energy', not {energy_type}"
        self.energy_type = energy_type

        if mace_name is None or mace_name == "mace":
            assert path is not None, 'For a non-foundation model, a path has to be specified.'
            model = torch.load(path, map_location="cpu")
        elif "off" in mace_name:
            from mace.calculators import foundations_models
            size = mace_name.split("-")[-1]
            model = foundations_models.mace_off(model=size, device="cpu", return_raw_model=True)
        elif "mp" in mace_name:
            from mace.calculators import foundations_models
            size = mace_name.split("-")[-1]
            model = foundations_models.mace_mp(model=size, device="cpu", return_raw_model=True)
        elif "ani" in mace_name:
            from mace.calculators import foundations_models
            model = foundations_models.mace_anicc(device="cpu", return_raw_model=True)

        if enable_cueq:
            print("Converting models to CuEq for acceleration")

            # This is a bit hacky, but due to current bugs in MACE and e3nn and changing the default dtype...
            default_device = torch.get_default_device()
            torch.set_default_device("cpu")
            model = run_cueq(model, device=default_device)
            torch.set_default_device(default_device)
            torch.set_default_dtype(self.dtype)

        self.model = model.to(torch.get_default_device(), self.dtype)
        self.mace_cutoff = self.model.r_max.cpu()

        if box is not None:
            self.linear_box = box
            self.box = torch.diag(box)
        else:
            self.linear_box = None
            self.box = None

        z_table = macetools.utils.AtomicNumberTable([int(z) for z in self.model.atomic_numbers])
        indices = macetools.atomic_numbers_to_indices(atomic_numbers.to(torch.device("cpu")).detach().numpy(), z_table=z_table)
        indices = torch.tensor(indices)
        node_attrs = macetools.to_one_hot(torch.tensor(indices, dtype=torch.long).unsqueeze(-1), num_classes=len(z_table)).to(self.dtype)

        ptr = torch.tensor([0, node_attrs.size(0)], dtype=torch.long, requires_grad=False)
        batch = torch.zeros(node_attrs.size(0), dtype=torch.long, requires_grad=False)
        pbc = torch.tensor([periodic, periodic, periodic])

        self.input_dict = dict(ptr=ptr,
                               node_attrs=node_attrs,
                               batch=batch,
                               pbc=pbc,
                               cell=self.box if self.box is not None else torch.zeros(3, 3))
    ...
    def update_box(self, box):
        ...
        self.linear_box = box
        self.box = torch.diag(box)
        self.input_dict["cell"] = self.box if self.box is not None else torch.zeros(3, 3)

    def calc_energy(self,
                    pos: torch.Tensor,
                    edgelist: torch.Tensor) -> torch.Tensor:
        """
        Calculate the energy of the system using the MACE model.
        ...
        edgelist : torch.Tensor
            List of atom pairs to consider for interactions

        Returns
        -------
        torch.Tensor
            Energy of the system in internal units
        """
        if self.atom_range is not None:
            pos = pos[self.atom_range[0]:self.atom_range[1]].to(self.dtype)
            if self.atom_range[0]!=0:
                edgelist = edgelist - self.atom_range[0]
        else:
            pos = pos.to(self.dtype)
        distances, shifts = get_distances_and_shifts(pos, edgelist, self.periodic, self.linear_box)

        distance_mask = distances < self.mace_cutoff

        edgelist = edgelist[:, distance_mask]
        shifts = shifts[distance_mask]

        idx_i_bi = torch.cat([edgelist[0], edgelist[1]])
        idx_j_bi = torch.cat([edgelist[1], edgelist[0]])

        edge_index = torch.stack([idx_i_bi, idx_j_bi])
        shifts = torch.cat([shifts, -shifts])

        self.input_dict["positions"] = pos * constants.internal_to_Angstrom
        self.input_dict["edge_index"] = edge_index
        self.input_dict["shifts"] = shifts

        energy = self.model(self.input_dict, compute_force=False)[self.energy_type]

        return energy * constants.eV_to_internal  # eV to internal unit
