...
for idx, temp in enumerate(temperatures):
    position_path = dimos.utils.project_path(f"examples/amber/{file}.rst7")
    if "gas" in file:
        positions = dimos.utils.read_positions(position_path)[:22].detach()
    else:
        positions = dimos.utils.read_positions(position_path)
    parameter_path = dimos.utils.project_path(f"examples/amber/{file}.prmtop")
    system = dimos.AmberForceField(parameter_path)
    integrator = dimos.LangevinDynamics(0.5, temp, 0.1, system)
    simulation = dimos.MDSimulation(system, integrator, positions, temperature=temp, seed=idx)
    simulations.append(simulation)

pot_energy_file = open("pot_energy_pt.dat","w")
kin_energy_file = open("kin_energy_pt.dat","w")
index_file = open("index_pt.dat","w")
acceptance_file = open("accepted_pt.dat","w")
num_accepted_file = open("num_accepted_pt.dat","w")



replica_exchange_ensemble = rex.ReplicaExchange(simulations, steps_before_exchange=20)
for t in range(5):
    print("Step", t)
    for idx, sim in enumerate(simulations):
        pot_energy_file.write(f"{sim.measure_potential_energy().item()}\t")
        kin_energy_file.write(f"{sim.measure_kinetic_energy().item()}\t")
        index_file.write(f"{replica_exchange_ensemble.indices[idx]}\t")
        acceptance_file.write(f"{replica_exchange_ensemble.accepted_swaps[idx]/replica_exchange_ensemble.attempted_swaps[idx]}\t")
        num_accepted_file.write(f"{replica_exchange_ensemble.accepted_swaps[idx]}\t")

    pot_energy_file.write("\n")
    kin_energy_file.write("\n")
    index_file.write("\n")
    acceptance_file.write("\n")
    num_accepted_file.write("\n")
    
    pot_energy_file.flush()
    kin_energy_file.flush()
    index_file.flush()
    acceptance_file.flush()
    num_accepted_file.flush()

    replica_exchange_ensemble.step(10)