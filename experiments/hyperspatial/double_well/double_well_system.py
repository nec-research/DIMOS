...
class DoubleWellEnergy(CommonBonded):
    """
    Double-well potential energy component.
    ...
    ----------
    a : float
        Barrier height parameter (default: 1.0)
    b : float
        Location of minima at x = ±b (default: 1.0)
    """
    ...
    def update_box(self, box):
        # No box dependence for this potential
        pass
    
    def __str__(self):
        return f"DoubleWell_a{self.a}_b{self.b}" # This needs to be unique for different parameters
    
    def calc_energy(self, pos: torch.Tensor) -> torch.Tensor:
        """
        Calculate the double-well potential energy.
        
        Parameters
        ----------
        pos : torch.Tensor
            Position tensor of shape (n_particles, dim)
        ...
        """
        r_squared = torch.sum(pos**2, dim=-1)
        energy_per_particle = self.a * (r_squared - self.b**2)**2
        return torch.sum(energy_per_particle)


class DoubleWellSystem(MinimalSystem):
    """
    A minimal system implementing a double-well potential.
    
    This system can be used for:
    - Standard parallel tempering
    - Replica exchange simulations
    - Hyperspatial replica exchange simulations (when wrapped with SeparableHyperspatialSystem)
    
    The Hamiltonian is: H(x) = a*(x^2 - b^2)^2
    ...
    dim : int
        Spatial dimension (default: 1 for 1D double-well)
    a : float
        Barrier height parameter (default: 1.0)
    b : float
        Location of minima at x = ±b (default: 1.0)
    mass : float
        Mass of particles (default: 1.0)
    box_size : float
        Size of the simulation box (default: 10.0)
    unit_system : str
        Unit system to use (default: "amber")
    create_graph : bool
        Whether to create computation graphs for autograd (default: False)
    """
    ...
    def __init__(
        ...
        self.num_atoms = n_particles  # Alias for compatibility with DIMOS
        self.a = a
        self.b = b
        self.mass_value = mass
        
        self.masses = torch.ones(n_particles) * mass
        
        # Set box for compatibility with hyperspatial systems
        self.box = torch.ones(dim) * box_size
        self.periodic = False  # Open boundary conditions
        
        self.use_neighborlist = False
        self.bonded_force_components.append(DoubleWellEnergy(a=a, b=b))
...
if __name__ == "__main__":
    # Simple test
    print("Testing DoubleWellSystem...")
    
    # Create a 1D double-well system with 1 particle
    system = DoubleWellSystem(n_particles=1, dim=1, a=1.0, b=1.0)
    
    # Test energy at different positions
    test_positions = [
        ("left minimum (x=-1)", torch.tensor([[-1.0]], requires_grad=True)),
        ("right minimum (x=+1)", torch.tensor([[1.0]], requires_grad=True)),
        ("barrier (x=0)", torch.tensor([[0.0]], requires_grad=True)),
        ("outside (x=2)", torch.tensor([[2.0]], requires_grad=True)),
    ]
    
    print("\nEnergy at different positions:")
    for name, pos in test_positions:
        energy = system.calc_energy(pos, neighborlist=None)
        print(f"  {name}: E = {energy.item():.4f}")
    
    # Test forces at x=0 (should be zero at barrier)
    pos_barrier = torch.tensor([[0.0]], requires_grad=True)
    energy, forces, acc = system.calc_energy(pos_barrier, neighborlist=None, return_forces=True)
    print(f"\nForce at barrier (x=0): F = {forces.item():.4f}")
    
    # Test forces at x=0.5 (should point toward left or right minimum)
    pos_mid = torch.tensor([[0.5]], requires_grad=True)
    energy, forces, acc = system.calc_energy(pos_mid, neighborlist=None, return_forces=True)
    print(f"Force at x=0.5: F = {forces.item():.4f}")
    
    print("\nDoubleWellSystem test passed!")
