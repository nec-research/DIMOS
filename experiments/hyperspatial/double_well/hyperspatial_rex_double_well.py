...
"""
Hyperspatial Replica Exchange for Double-Well System.

This script runs hyperspatial replica exchange (HREX) simulations for
the double-well potential using the BatchedHyperspatialReplicaExchange
class from DIMOS.

Output files contain:
- Trajectories (x and z coordinates for each replica)
- Energies (total and projected d=1)
...
from different_mu_simulation import DoubleWellSimulations
...
class HREXRunner:
    """
    Runner for hyperspatial replica exchange simulations of the double-well system.
    ...
    def __init__(
        ...
        self.simulations = DoubleWellSimulations(
            mu_values=mu_values,
        ...
        # Setup batching simulations for HREX
        self.separable_sim = self.simulations.simulations[0]
        self.multi_sim = MultiSim(self.simulations.simulations[1:])

        well_energy = self.multi_sim.sys.bonded_force_components[0]
        extra_dim_energy = self.multi_sim.sys.bonded_force_components[1]

        # Combine into hyperspatial setup
        self.hrex_multi_sim = hrex.HyperspatialReplicaExchangeMultiSim(
            self.separable_sim, self.multi_sim
        )
        
        # And finally setup multi-replica HREX
        self.batched_rex = hrex.BatchedHyperspatialReplicaExchange(
            self.hrex_multi_sim,
            steps_before_exchange=steps_before_exchange,
            compile=compile,
        )
        
        self.num_replicas = len(mu_values) + 1  # +1 for the separable replica
    ...
    def run(
        ...
        """
        Run the HREX simulation.
        
        Parameters
        ----------
        n_steps : int
            Total number of HREX steps (each step = steps_before_exchange dynamics + exchange attempt)
        save_interval : int
        ...
        output_path = os.path.join(output_dir, f"hrex_{timestamp}")
        os.makedirs(output_path, exist_ok=True)
        
        # Reset exchange statistics
        self.batched_rex.reset_acceptance_statistics()
        ...
        for step in range(n_steps):
            # Perform HREX step (dynamics + exchange attempts)
            self.batched_rex.step(steps=1, detach=True)

            if step % save_interval == 0:
                output_file_x.write(f"{step}\t")
                output_file_z.write(f"{step}\t")
                for idx in range(self.num_replicas):
                    pos = self.batched_rex.get_pos(idx)
                    output_file_x.write(f"{pos[0,0].item()}\t")
                    output_file_z.write(f"{pos[0,1].item()}\t")
                output_file_x.write("\n")
                output_file_z.write("\n")
    ...
    def print_final_statistics(self, output_path: str = None):
        ...
        attempted = self.batched_rex.attempted_swaps
        accepted = self.batched_rex.accepted_swaps
        
        for i in range(len(attempted)):
            rate = accepted[i].item() / attempted[i].item() if attempted[i].item() > 0 else 0.0
            stats_lines.append(f"Replica pair {i}-{i+1}:")
