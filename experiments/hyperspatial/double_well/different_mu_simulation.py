...
"""
Batched simulation for double-well system with different mu penalties.

This script runs simulations for:
- Original d=1 system (base double-well)
- Extended d=2 systems (hyperspatial) with mu = 1000, 100, 10, 1, 0.1

Uses Langevin dynamics for propagation. All simulations are run using
MDSimulation with Langevin dynamics integrator.

Output files contain x and z coordinates separately.
"""
...
from double_well_system import DoubleWellSystem


class DoubleWellSimulations:
    """
    A batched simulation object for running multiple double-well systems
    with different mu penalties using Langevin dynamics.
    
    Can be used for:
    - Independent simulations (current implementation)
    - Hyperspatial replica exchange (future extension)
    
    Parameters
    ----------
    mu_values : list
        List of mu penalty values for the extra dimension
    temperature : float
        Simulation temperature
    a : float
        Double-well barrier height parameter
    b : float
        Double-well minima location parameter
    seed : int
    ...
    def __init__(
        self,
        mu_values: list = [1000.0, 100.0, 10.0, 1.0, 0.1],
        ...
        # Initialize constants for unitless system
        constants.init_constants_in_unit_system("unitless")
        
        # Create simulations
        self.simulations = []
        self.one_d_sim = None
        self.one_d_system = None
        self.hyperspatial_systems = []  # Store references to hyperspatial systems for projected energy calculation
        
        # Create the base d=1 system
        base_system = DoubleWellSystem(
            n_particles=1,
            dim=1,
            a=self.a,
            b=self.b,
        )
        self.one_d_system = base_system  # Store reference
        
        #True d=1 system
        initial_pos = torch.tensor([-self.b], requires_grad=True)
        integrator = LangevinDynamics(timestep=self.timestep,temperature=self.temperature,friction=self.friction,sys=base_system)
        simulation = MDSimulation(sys=base_system,integrator=integrator,initial_pos=initial_pos,temperature=self.temperature,seed=self.seed)
        self.one_d_sim = simulation

        # Separable System
        base_system = DoubleWellSystem(n_particles=1,dim=1,a=self.a,b=self.b)
        seperable_hyper_system = SeparableHyperspatialSystem(system=base_system,num_extra_dim=1,penalty_extra_dim=mu_values[0])
        self.hyperspatial_systems.append((seperable_hyper_system, base_system))
        initial_pos = torch.tensor([[-self.b, 0.0]], requires_grad=True)
        integrator = LangevinDynamics(timestep=self.timestep,temperature=self.temperature,friction=self.friction,sys=seperable_hyper_system)
        simulation = MDSimulation(sys=seperable_hyper_system,integrator=integrator,initial_pos=initial_pos,temperature=self.temperature,seed=self.seed + 1)
        self.simulations.append(simulation)

        for idx, mu in enumerate(mu_values):
            base_system = DoubleWellSystem(n_particles=1,dim=1,a=self.a,b=self.b)
            hyper_system = HyperspatialSystem(system=base_system,num_extra_dim=1,penalty_extra_dim=mu)
            self.hyperspatial_systems.append((hyper_system, base_system))  # Store system and wrapped base
            initial_pos = torch.tensor([[-self.b, 0.0]], requires_grad=True)
            integrator = LangevinDynamics(timestep=self.timestep,temperature=self.temperature,friction=self.friction,sys=hyper_system)
            simulation = MDSimulation(sys=hyper_system,integrator=integrator,initial_pos=initial_pos,temperature=self.temperature,seed=self.seed + idx + 2)
            self.simulations.append(simulation)


    
    def run(self, n_steps: int, save_interval: int = 100, output_dir: str = "output"):
        """
        Run all simulations for n_steps.
        
        Parameters
        ----------
        n_steps : int
            Total number of Langevin dynamics steps
        save_interval : int
            Interval at which to save coordinates
        output_dir : str
        ...
        print(f"Running {len(self.simulations) + 1} simulations for {n_steps} steps")
        print(f"Using Langevin dynamics with timestep={self.timestep}, friction={self.friction}")
        print(f"Output directory: {output_path}")
        print()

        # Save simulation parameters
        params_file = os.path.join(output_path, "parameters.txt")
        with open(params_file, 'w') as f:
            f.write(f"# Simulation parameters\n")
            f.write(f"temperature = {self.temperature}\n")
            f.write(f"a = {self.a}\n")
            f.write(f"b = {self.b}\n")
            f.write(f"mu_values = {self.mu_values}\n")
            f.write(f"timestep = {self.timestep}\n")
            f.write(f"friction = {self.friction}\n")
            f.write(f"seed = {self.seed}\n")
            f.write(f"save_interval = {save_interval}\n")

        true_one_d_output = os.path.join(output_path, "true_1d_trajectory.dat")
        true_one_d_output_file = open(true_one_d_output, 'w')

        trajectories_x = os.path.join(output_path, "trajectories_x.dat")
        output_file_x = open(trajectories_x, 'w')

        trajectories_z = os.path.join(output_path, "trajectories_z.dat")
        output_file_z = open(trajectories_z, 'w')

        true_energy_output = os.path.join(output_path, "true_1d_energies.dat")
        true_energy_output_file = open(true_energy_output, 'w')

        projected_energy_output = os.path.join(output_path, "projected_d1_energies.dat")
        projected_energy_output_file = open(projected_energy_output, 'w')

        system_energy_output = os.path.join(output_path, "system_energies.dat")
        system_energy_output_file = open(system_energy_output, 'w')
        
        # Run simulations
        for step in range(n_steps):
            # Step 1D simulation
            self.one_d_sim.step(1)
            ...
            if step % save_interval == 0:
                ...
                true_one_d_output_file.write(f"{step} {self.one_d_sim.pos[0,0].item()}\n")

                output_file_x.write(f"{step}\t")
                output_file_z.write(f"{step}\t")
                for sim in self.simulations:
                    output_file_x.write(f"{sim.pos[0,0].item()}\t")
                    output_file_z.write(f"{sim.pos[0,1].item()}\t")
                output_file_x.write("\n")
                output_file_z.write("\n")

                # print true energy and projected energies
                projected_energy_output_file.write(f"{step}\t")
                true_energy_output_file.write(f"{step}\t")
                system_energy_output_file.write(f"{step}\t")
                for idx, sim in enumerate(self.simulations):
                    pos = sim.pos
                    hyper_system = self.hyperspatial_systems[idx][0]
                    base_system = self.hyperspatial_systems[idx][1]
                    pos_1d = pos.clone()
                    pos_1d[:,1:] = 0.0  # Project to d=1 by zeroing extra dimensions
                    projected_energy = base_system.calc_energy(pos_1d, None)
                    true_energy = base_system.calc_energy(pos, None)
                    system_energy = hyper_system.calc_energy(pos, None)
                    projected_energy_output_file.write(f"{projected_energy.item()}\t")
                    true_energy_output_file.write(f"{true_energy.item()}\t")
                    system_energy_output_file.write(f"{system_energy.item()}\t")

                projected_energy_output_file.write("\n")
                true_energy_output_file.write("\n")
                system_energy_output_file.write("\n")
...
if __name__ == "__main__":  
    # Create batched simulation with different mu values
    batch_sim = DoubleWellSimulations(
        mu_values=[1000.0, 100.0, 10.0, 1.0, 0.1, 0.01, 0.001],
        temperature=0.1,
        a=1.0,
        b=1.0,
        timestep=0.01,  # Small timestep for Langevin dynamics
        friction=1.0,   # Friction coefficient in fs^-1
        seed=42,
    )
