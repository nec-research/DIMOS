...
"""
ORB machine learning potential module.

This module provides integration with ORB machine learning potentials for molecular simulations.

The module contains the following main classes:

- OrbFoundationalSystem: System class that implements ORB foundational models
- OrbEnergyContribution: Energy calculation class using ORB potentials

These classes enable molecular dynamics simulations using pre-trained ORB models,
with support for periodic boundary conditions, neighbor lists, and different unit systems.
The implementation handles unit conversions between ORB's native units (Angstroms, eV)
and DIMOS internal units.
"""

import torch

from dimos.utils import get_distances_and_vectors
from dimos.ff import MinimalSystem
from dimos import constants

from orb_models.forcefield import base, pretrained

class OrbFoundationalSystem(MinimalSystem):
    """
    System class for using Orb foundational models.

    This class implements the Orb foundational models for molecular simulations,
    providing an interface between DIMOS and the Orb machine learning potentials.
    ...
    orb_name : str, optional
        Name of the Orb model to use, default is "orb_v2"
    ...
    def __init__(
        ...
        super().__init__(unit_system, dtype)
        ...
        orb_energy_contribution = OrbEnergyContribution(atomic_numbers, periodic=periodic, orb_name=orb_name, box=box, dtype=self.dtype)

        self.nonbonded_force_components.append(orb_energy_contribution)
        self.cutoff = orb_energy_contribution.orb_cutoff


class OrbEnergyContribution():
    """
    Energy contribution from Orb machine learning potentials.

    This class implements the energy calculation using Orb foundational models.
    See https://github.com/orbital-materials/orb-models for more details about Orb.
    ...
    orb_name : str, optional
        Name of the Orb model to use, default is 'orb_v2'
    orb_cutoff : float, optional
        Cutoff distance for Orb interactions, default is 6.0
    dtype : torch.dtype, optional
        Data type for tensors, default is torch default dtype
    ...
    model : object
        The loaded Orb model
    orb_cutoff : float
        Cutoff distance for interactions
    linear_box : torch.Tensor
        Box vectors in linear form
    box : torch.Tensor
        Box vectors in diagonal matrix form
    atomic_numbers : torch.Tensor
        Atomic numbers of atoms
    atom_type_embedding : torch.Tensor
    ...
    def __init__(self,
                 ...
                 periodic: bool = False,
                 orb_name: str = 'orb_v2',
                 orb_cutoff: float = 6.0,
                 dtype=torch.get_default_dtype()):

        self.periodic = periodic
        self.dtype = dtype
        if orb_name is None or orb_name == "orb":
            raise NotImplementedError("Currently, dimos only supports foundational models of orb")

        orb_name = orb_name.replace("-", "_")
        model = getattr(pretrained, orb_name)(device=torch.get_default_device())
        self.model = model.to(torch.get_default_device(), self.dtype)

        self.orb_cutoff = orb_cutoff
        ...
        self.atom_type_embedding = torch.nn.functional.one_hot(
    ...
    def calc_energy(self,
        ...
        """
        Calculate the energy using the Orb model.
        ...
        -------
        torch.Tensor
            The calculated energy in internal units
        """
        pos = pos.to(self.dtype)
        distances, vectors = get_distances_and_vectors(pos, edgelist, self.periodic, self.linear_box)

        distance_mask = distances < self.orb_cutoff
        ...
        edge_features = {"r": distances, "vectors": vectors}
        system_features = {"cell": self.box.unsqueeze(0)}
        node_features = {
            "positions": pos,
            "atomic_numbers": self.atomic_numbers,
            "atomic_numbers_embedding": self.atom_type_embedding.to(
                torch.get_default_dtype())}

        input_graph = base.AtomGraphs(
            senders=edge_index[0],
            receivers=edge_index[1],
            n_node=torch.tensor([pos.size(0)]),
            n_edge=torch.tensor([edge_index[0].size(0)]),
        ...
        result = self.model.predict(input_graph)

        return result["graph_pred"] * constants.eV_to_internal  # TODO: Not sure about units of Orb...
