...
"""
Standard Temperature Parallel Tempering for Double-Well System.

This script runs standard parallel tempering (temperature replica exchange)
simulations for the double-well potential using either:
- ReplicaExchange class (standard mode): Individual simulations stepped separately
- BatchedReplicaExchange (batched mode): Simulations batched together via MultiSim for better GPU utilization

Output files contain:
- Trajectories (x coordinates for each replica)
- Energies (potential energy for each replica)
- Replica indices (tracking which configuration is in which replica)
- Exchange statistics (acceptance rates)
"""
...
import dimos.advanced_methods.batched_replica_exchange as batched_rex
...
class PTRunner:
    """
    Runner for parallel tempering simulations of the double-well system.
    
    Parameters
    ----------
    temperatures : list
        List of temperatures for the replicas (from low to high)
    a : float
    ...
    steps_before_exchange : int
        Number of Langevin steps between exchange attempts
    compile : bool
        Whether to compile the step functions for performance
    batched : bool
        Whether to use batched replica exchange (MultiSim) for better GPU utilization
    """
    ...
    def __init__(
        ...
        timestep: float = 0.01,
        friction: float = 1.0,
        steps_before_exchange: int = 10,
        compile: bool = False,
        batched: bool = False,
    ):
        # Generate temperature ladder if not provided
        if temperatures is None:
            # Exponential temperature spacing for better exchange rates
            temperatures = [
        ...
        for idx, temp in enumerate(temperatures):
            system = DoubleWellSystem(
                n_particles=1,
                dim=1,
                a=a,
                b=b,
            )
            # Start at the left minimum
            initial_pos = torch.tensor([[-b]], requires_grad=True)
        ...
        # Setup replica exchange - choose between standard and batched modes
        if batched:
            # Batched mode: uses MultiSim for better GPU utilization
            self.multi_sim = MultiSim(self.simulations, seed=seed)
            self.replica_exchange = batched_rex.ParallelTempering(
                self.multi_sim,
                steps_before_exchange=steps_before_exchange,
                compile=compile,
            )
        else:
            # Standard mode: individual simulations stepped separately
            self.replica_exchange = rex.ReplicaExchange(
                self.simulations,
                steps_before_exchange=steps_before_exchange,
                parallel=False,  # For small systems, parallel overhead isn't worth it
                compile=compile,
            )
    ...
    def run(
        ...
        ----------
        n_steps : int
            Total number of PT steps (each step = steps_before_exchange dynamics + exchange attempt)
        save_interval : int
            Interval at which to save coordinates and energies
        output_dir : str
        ...
        output_path = os.path.join(output_dir, f"pt_{timestamp}")
        os.makedirs(output_path, exist_ok=True)
        
        # Reset exchange statistics
        self.replica_exchange.reset_acceptance_statistics()

        # Save parameters
        params_file = os.path.join(output_path, "parameters.txt")
        with open(params_file, 'w') as f:
            f.write(f"# Simulation parameters\n")
            f.write(f"temperatures = {self.temperatures}\n")
            f.write(f"n_replicas = {self.n_replicas}\n")
            f.write(f"a = {self.a}\n")
            f.write(f"b = {self.b}\n")
            f.write(f"timestep = {self.timestep}\n")
            f.write(f"friction = {self.friction}\n")
            f.write(f"steps_before_exchange = {self.steps_before_exchange}\n")
            f.write(f"seed = {self.seed}\n")
            f.write(f"save_interval = {save_interval}\n")
            f.write(f"n_steps = {n_steps}\n")
            f.write(f"batched = {self.batched}\n")

        # Open output files
        trajectories_file = os.path.join(output_path, "trajectories_x.dat")
        output_file_x = open(trajectories_file, 'w')

        energy_file = os.path.join(output_path, "energies.dat")
        energy_output = open(energy_file, 'w')

        indices_file = os.path.join(output_path, "replica_indices.dat")
        indices_output = open(indices_file, 'w')
        
        # Run simulation
        for step in range(n_steps):
            # Perform PT step (dynamics + exchange attempts)
            self.replica_exchange.step(steps=1, detach=True)
            ...
            if step % save_interval == 0:
                ...
                if self.batched:
                    for idx in range(self.n_replicas):
                        pos = self.multi_sim.get_pos(idx)
                        output_file_x.write(f"{pos[0, 0].item()}\t")
                else:
                ...
                # Save energies
                energy_output.write(f"{step}\t")
                if self.batched:
                    for idx in range(self.n_replicas):
                        pos = self.multi_sim.get_pos(idx)
                        energy = self.simulations[idx].sys.calc_energy(pos, None)
                        energy_output.write(f"{energy.item()}\t")
                else:
                    for sim in self.simulations:
                ...
                # Save replica indices
                indices_output.write(f"{step}\t")
                for idx in self.replica_exchange.indices:
                    indices_output.write(f"{idx.item()}\t")
                indices_output.write("\n")
        ...
        output_file_x.close()
        energy_output.close()
        indices_output.close()
        
        self.print_final_statistics(output_path=output_path)
        
        return output_path
    
    def print_final_statistics(self, output_path: str = None):
        """
        Print final exchange statistics to terminal and optionally to file.
        
        Parameters
        ----------
        output_path : str, optional
            Directory path to save statistics file. If None, only prints to terminal.
        """
        stats_lines = []
        stats_lines.append("Final Exchange Statistics:")
        stats_lines.append("-" * 60)
        
        attempted = self.replica_exchange.attempted_swaps
        accepted = self.replica_exchange.accepted_swaps
        
        for i in range(len(attempted)):
            rate = accepted[i].item() / attempted[i].item() if attempted[i].item() > 0 else 0.0
            stats_lines.append(f"Replica pair {i}-{i+1} (T={self.temperatures[i]:.4f} <-> T={self.temperatures[i+1]:.4f}):")
            stats_lines.append(f"    Attempted: {int(attempted[i].item())}")
            stats_lines.append(f"    Accepted:  {int(accepted[i].item())}")
            stats_lines.append(f"    Rate:      {rate:.4f}")
        stats_lines.append("-" * 60)
        ...
        if output_path is not None:
            stats_file = os.path.join(output_path, "exchange_statistics.txt")
...
if __name__ == "__main__":
    ...
    n_replicas = 2
    T_min = 0.05  # Low temperature - mostly confined to one well
    T_max = 0.5   # High temperature - can easily cross the barrier
    a = 1.0  # Barrier height
    b = 1.0  # Minima locations at +/- b
    
    # Dynamics parameters
    timestep = 0.01
    friction = 1.0
    steps_before_exchange = 10
    
    # Simulation length
    n_steps = 100000  # Number of exchange steps
    save_interval = 10
    
    # Use batched mode for better GPU utilization (recommended for larger systems)
    batched = False
    ...
    print(f"\nSimulation complete. Results in: {output_path}")
