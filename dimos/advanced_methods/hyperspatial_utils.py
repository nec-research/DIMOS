...
"""
Hyperspatial system utilities for enhanced sampling.

This module provides classes for extending molecular systems with extra dimensions
(hyperspatial extension) to improve sampling in replica exchange simulations.

Classes:
    ExtraDimensionPenaltyEnergy: Energy component for harmonic penalty on extra dimensions
    HyperspatialSystem: Wrapper that extends a system with extra dimensions (non-separable)
    SeparableHyperspatialSystem: Wrapper with separable energy contribution from extra dims
"""
...
from dimos.energy import CommonBonded
import dimos

_LOG2 = math.log(2.0)
_EXTRA_DIM_FORMS = ("harmonic", "log_cosh")


def _make_extra_dim_penalty(
    num_extra_dim: int,
    penalty_extra_dim: Union[float, torch.Tensor],
    extra_box: torch.Tensor,
    mask: Optional[torch.Tensor] = None,
    form: str = "harmonic",
    k: float = 0.5,
) -> "ExtraDimensionPenaltyEnergy":
    return ExtraDimensionPenaltyEnergy(
        num_extra_dim,
        penalty_extra_dim,
        extra_box,
        mask=mask,
...
class ExtraDimensionPenaltyEnergy(CommonBonded):
    """
    Penalty energy for extra dimensions in hyperspatial systems.
    ...
    log_cosh (saturating; matches harmonic for small |z|):
        E = sum (mu / k^2) * log(cosh(k * z))

    For |k*z| << 1, log-cosh reduces to 0.5 * mu * z^2.  At large |z| the
    restoring force saturates at mu/k instead of growing without bound.

    Parameters
    ----------
    num_extra_dim : int
        Number of extra dimensions added to the system
    penalty_extra_dim : float or torch.Tensor
        Penalty strength (mu) for the extra dimensions
    extra_box : torch.Tensor
        Box size for periodic corrections in extra dimensions
    mask : torch.Tensor, optional
        Boolean mask to apply penalty only to selected atoms.
    form : str
        ``"harmonic"`` or ``"log_cosh"``
    k : float
        Inverse width (1/Angstrom) for log-cosh; transition near |z| ~ 1/k.
    """
    ...
    def __init__(
        ...
        form = form.lower()
        if form not in _EXTRA_DIM_FORMS:
            raise ValueError(f"form must be one of {_EXTRA_DIM_FORMS}, got {form!r}")
        if k <= 0:
            raise ValueError(f"k must be positive, got {k}")

        self.num_extra_dim = num_extra_dim
        self.penalty_extra_dim = penalty_extra_dim
        self.extra_box = extra_box
        self.mask = mask
        self.form = form
        self.k = k

    def update_box(self, box):
        raise NotImplementedError("Box changes for the hyperspatial system currently not implemented")

    def __str__(self):
        mask_str = "_masked" if self.mask is not None else ""
        return f"ExtraDimPenalty_{self.form}_{self.num_extra_dim}_{self.k}{mask_str}"

    def _extra_coords(self, pos: torch.Tensor) -> torch.Tensor:
        if self.mask is not None:
            return pos[self.mask, -self.num_extra_dim:]
        return pos[:, -self.num_extra_dim:]

    def calc_energy(self, pos: torch.Tensor) -> torch.Tensor:
        """
        Calculate the extra-dimension penalty energy.

        Parameters
        ----------
        pos : torch.Tensor
            Position tensor of shape (n_atoms, dim) where dim includes extra dimensions

        Returns
        -------
        torch.Tensor
            Scalar penalty energy
        """
        dist_to_zero = dimos.periodic_correction(
            self._extra_coords(pos), self.extra_box
        )
        mu = self.penalty_extra_dim

        if self.form == "harmonic":
            return 0.5 * torch.sum(mu * (dist_to_zero) ** 2)

        kz = self.k * dist_to_zero
        log_cosh_kz = torch.logaddexp(kz, -kz) - _LOG2
        return torch.sum((mu / (self.k ** 2)) * log_cosh_kz)


class HyperspatialSystem:
    """
    Wrapper that extends a molecular system with extra dimensions (non-separable).
    
    In the non-separable case, the extra dimension penalty is added directly to the
    system's bonded force components, so the base system's energy function handles
    all interactions including the extra dimensions.
    
    Parameters
    ----------
    system : MinimalSystem or similar
        The base molecular system to extend
    num_extra_dim : int
        Number of extra dimensions to add
    penalty_extra_dim : float or tuple
        Harmonic penalty strength(s). If a single float, applied uniformly.
        If a tuple of (penalty_protein, penalty_solvent), different penalties
        are applied based on protein_mask.
    protein_mask : torch.Tensor, optional
        Boolean mask indicating protein atoms (True) vs solvent (False).
        Required if penalty_extra_dim is a tuple.
    extra_dim_form : str
        ``"harmonic"`` or ``"log_cosh"`` (see ExtraDimensionPenaltyEnergy).
    extra_dim_k : float
        log-cosh width parameter in 1/Angstrom (default 0.5).
    """
    ...
    def __init__(
        ...
        system,
        num_extra_dim: int,
        penalty_extra_dim: Union[float, Tuple[float, float]],
        protein_mask: Optional[torch.Tensor] = None,
        extra_dim_form: str = "harmonic",
        extra_dim_k: float = 0.5,
    ):
        self.sys = system
        ...
        if system.box is None:
            ...
            box = torch.nn.functional.pad(system.box, (0, num_extra_dim))
            box[-num_extra_dim:] = torch.mean(system.box)
            self.sys.update_box(box)

        self.dim = self.sys.dim + num_extra_dim

        if box is not None:
            if protein_mask is not None:
                # Separate penalties for protein and solvent
                if isinstance(penalty_extra_dim, (tuple, list)):
                    penalty_protein, penalty_solvent = penalty_extra_dim
                else:
                    penalty_protein = penalty_solvent = penalty_extra_dim
                
                self.sys.bonded_force_components.append(
                    _make_extra_dim_penalty(
                        num_extra_dim,
                        penalty_protein,
                        box[-num_extra_dim:],
                        mask=protein_mask,
            ...
            # TODO: Currently supports only one penalty component if no box
            self.sys.bonded_force_components.append(
                _make_extra_dim_penalty(
                    num_extra_dim,
                    penalty_extra_dim,
    ...
    def __getattr__(self, name):
        # Forward any unknown attributes to the wrapped system
        # Use object.__getattribute__ to avoid infinite recursion
        sys = object.__getattribute__(self, 'sys')
        return getattr(sys, name)

    def apply_boundary_conditions(self, pos: torch.Tensor) -> torch.Tensor:
        """
        Apply boundary conditions only to the physical dimensions.

        The extra dimensions should NOT be wrapped periodically, as they use
        a penalty to confine atoms near the origin, not periodic boundary conditions.
        """
        if self.sys.box is None:
            return pos
        # Get the number of extra dimensions
        # num_extra_dim = object.__getattribute__(self, 'dim') - self.sys.dim
        # Only wrap the physical dimensions (first self.sys.dim dimensions)
        return_pos = pos.clone()
        physical_pos = pos[:, :self.sys.dim]
        physical_box = self.sys.box[:self.sys.dim]
        boundaried_pos = physical_pos - torch.floor(physical_pos / physical_box) * physical_box
        return_pos[:, :self.sys.dim] = boundaried_pos
        return return_pos
    ...
    def calc_energy(
        ...
        return self.sys.calc_energy(pos, neighborlist, return_forces, print_energies)


class SeparableHyperspatialSystem:
    """
    Wrapper with separable energy contribution from extra dimensions.
    
    In the separable case, the extra dimension penalty is computed separately
    from the base system's energy. This allows the base system to operate only
    on the physical dimensions while the extra dimension energy is added on top.
    
    This is useful for replica exchange where the exchange criterion can exploit
    ...
    def __init__(
        ...
        if system.box is None:
            extra_box = 100  # Match HyperspatialSystem behavior for non-periodic systems
        else:
            extra_box = torch.mean(system.box).repeat(num_extra_dim)
    ...
    def apply_boundary_conditions(self, pos: torch.Tensor) -> torch.Tensor:
        ...
        if self.sys.box is None:
            return pos
        return_pos = pos.clone()
        boundaried_pos = self.sys.apply_boundary_conditions(pos[:, :self.sys.dim])
        return_pos[:, :self.sys.dim] = boundaried_pos
        return return_pos
    ...
    def calc_energy(
        ...
        """
        Calculate total energy including base system and extra dimension penalty.
        
        Parameters
        ----------
        pos : torch.Tensor
            Position tensor of shape (n_atoms, dim) including extra dimensions
        neighborlist : optional
            Neighborlist for the base system (can be None for simple systems)
        return_forces : bool
            If True, also return forces and accelerations
        print_energies : bool
            If True, print energy components (passed to base system)
            
        Returns
        -------
        torch.Tensor or tuple
            Total energy, or (energy, forces, acceleration) if return_forces=True
        """
        # Base system energy (only physical dimensions)
        energy = self.sys.calc_energy(
            pos[:, :self.sys.dim], neighborlist, return_forces=False, print_energies=print_energies
        )
        
        # Extra dimension penalties
        extra_energy = sum(contrib.calc_energy(pos) for contrib in self.extra_energy_contrib)
        total_energy = energy + extra_energy
        
        if return_forces:
            if self.sys.create_graph:
                forces = -torch.autograd.grad(
                    total_energy.sum(), pos, retain_graph=True, create_graph=True
                )[0]
            else:
                forces = -torch.autograd.grad(
                    total_energy.sum(), pos, retain_graph=False, create_graph=False
                )[0]

            acceleration = forces / self.masses.unsqueeze(-1)
            return total_energy, forces, acceleration
